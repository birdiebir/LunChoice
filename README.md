...
- **走路時間上限**：2–10 分鐘（以座標計算實際步行分鐘）
  - **自訂出發點（實驗性功能，只影響預設轉盤）**：「今日條件」面板可以按「📍 使用目前位置」，用瀏覽器定位（Geolocation API）重算預設轉盤 405 家的走路時間（直線距離換算，非實際路線，跟共享轉盤自動帶入同一套公式）；重新整理頁面會回到預設出發點（基隆路一段 200 號），不會存起來。因為餐廳名單本來就是圍繞固定地點收錄的，換到離原本地點很遠的位置，符合走路時間上限的店家可能會變成 0 家——這是資料本身的限制，不是 bug。共享轉盤不受影響，地點維持使用者新增/編輯當下自己存的走路時間。
