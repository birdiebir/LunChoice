...
- **看廣告多轉一次**：3 次用完後轉盤上會出現提示，點下去顯示一張滿版靜態廣告圖（遠銀 Bankee，示意用，點圖另開分頁到廣告連結），2 秒後才能關閉並換得當天 +1 次額度，每人每天限領一次，額度同樣由後端強制執行
  - 網址加 `?adtest=1`（例如 `https://lunchoice.netlify.app/?adtest=1`）會進入廣告測試模式：轉盤上的廣告按鈕永遠顯示、可以無限次點開廣告視窗檢查素材／版面，不受「次數用完」與「今天已經領過」限制。後端每人每天仍然只會真的核發一次額度，測試模式不會讓額度被無限重複領取。
