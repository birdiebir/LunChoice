...
- **雙轉盤：預設轉盤／共享轉盤**：轉盤上方有頁籤可以切換。「預設轉盤」就是原本這 405 家的固定名單；「共享轉盤」是全站共用、一開始空白的名單，任何登入使用者新增的地點會透過 Supabase Realtime 即時同步給所有正在瀏覽的人，不用重新整理頁面。切到共享轉盤時會出現「+ 新增地點到共享清單」按鈕，表單欄位是店名／類別／價位／走路時間／Google Maps 網址；貼 Google Maps 網址按「自動帶入」可以解析出店名與經緯度、並用直線距離估算走路時間（不是真正的路線時間，可自行調整），價位跟類別 Google 沒有公開在網址裡、需要手動選
  - **編輯共享地點**：切到共享轉盤時，展開「看看今天名單裡有哪些店」清單，每張卡片右上角有個 ✎ 編輯圖示，點開會用同一個表單帶入現有資料，改完按「儲存變更」就會更新（維基式共同維護，任何登入使用者都能改任何一筆，不限本人新增的）；改完自己畫面立刻更新，其他人那邊靠 Realtime 同步過去。**預設轉盤的 405 家沒有這個圖示**——那份資料是寫死在 `index.html` 裡的固定清單，不是資料庫，本來就沒有「更新」這回事
- **個人檔案**：右上角「個人檔案」可以改暱稱、上傳大頭照（存在 Supabase Storage，公開讀取、只能改自己的檔案），下面列出自己過去每一次轉盤的日期與轉到的項目
...
部署後開啟網頁 → 輸入 email 與密碼 → 點「登入 / 註冊」（沒帳號會自動建立）→ 開始轉盤。右上角會顯示「今天還可轉 N/3 次」。轉盤上方可以切換「預設轉盤」／「共享轉盤」；右上角可以進個人檔案、建立或加入飯搭子圈。
