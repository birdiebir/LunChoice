...
- 全部彈窗（結果卡、登入、廣告、新增/編輯地點、我的檔案、飯搭子圈）共用 `openVeil()`／`closeVeil()`：開窗時把背景 `.app` 設成 `inert`（鍵盤 Tab 不到、不會誤觸背景按鈕），關窗才解除；`aria-hidden` 也會跟著同步。
- 空白鍵轉盤快捷鍵改用「目前有沒有任何彈窗開著」（`openModalCount`）當條件，並排除 `SELECT`／`BUTTON` 取得焦點的情況——原本只排除 `.veil`／`.gate` 兩個彈窗，其餘彈窗開著時按空白鍵會在背景偷偷轉一次；焦點在按鈕上按空白鍵也會誤觸多轉一次。
- 單一輸入框的表單（群組名稱、加入成員 email、暱稱、Google Maps 快速帶入）補上 Enter 送出；多欄位的新增地點表單維持不加，避免填一半被 Enter 誤送。

## 功能

- **預算上限**：NT$100–500（以人均平均消費過濾）
- **走路時間上限**：2–10 分鐘（以座標計算實際步行分鐘）
  - **自訂出發點（實驗性功能）**：「今日條件」面板可以在「基隆路一段 200 號」／「目前位置」兩個頁籤切換。選「目前位置」會用瀏覽器定位（Geolocation API）取得座標，同時重算**預設轉盤與共享轉盤**兩邊的走路時間（直線距離換算，非實際路線）；並用免費的 OpenStreetMap Nominatim 服務反查文字地址顯示出來（純前端呼叫，沒有金鑰，查詢失敗只是顯示不出地址文字，不影響走路時間照算）。切回「基隆路一段 200 號」則兩邊都恢復用原本存的走路時間（預設轉盤是人工判別過的、共享轉盤是新增/編輯當下依當時出發點算出來存的）。重新整理頁面會回到預設出發點，不會存起來。因為預設轉盤那 405 家本來就是圍繞固定地點收錄的，換到離原本地點很遠的位置，符合走路時間上限的店家可能會變成 0 家——這是資料本身的限制，不是 bug。
- **11 種類別篩選**：麵食、飯食便當、日式、韓式、東南亞、西式、台式小吃、健康餐盒、鍋物、咖啡輕食、其他
- 轉出結果附價位、步行時間、Google 地圖一鍵導航；篩選設定記在瀏覽器
...
- **看廣告多轉一次，可無限次領取**：3 次用完後轉盤上會出現提示，點下去顯示一張滿版靜態廣告圖（遠銀 Bankee，示意用，點圖另開分頁到廣告連結），2 秒後才能關閉並換得 +1 次額度——每個帳號都可以一直重複看廣告、一直加轉，沒有每日上限，「每天 3 次」只是免費的基礎額度
  - 廣告素材有兩張，輪流出現（不是隨機，是每開一次照順序換下一張）：`assets/bonus-offer.jpg`（數位帳戶開戶）連到開戶頁、`assets/bonus-offer-2.jpg`（12 個月定存 1.8%）連到 `event/cd180Q2` 頁；輪替清單是 `script.js` 裡的 `AD_CREATIVES` 陣列，要加減素材改那邊就好
...
- **Header 版位**：主標題「午餐大轉輪」在左，導覽（飯搭子圈、我的檔案）在右，兩邊水平置中對齊；「今天還可轉 N 次」的額度徽章放在轉盤正上方置中；大頭照＋email 在導覽最右邊，點下去才展開「登出」選單（原本是常駐的登出按鈕）
...
5. 後台 **Project Settings → API** → 複製 **Project URL** 與 **anon public key**，填進 `script.js` 最上方的：
   ...
   const DAILY_LIMIT = 3;   // 只是畫面初始顯示用；真正的每日上限寫死在 supabase/schema.sql 的 record_spin()/spin_status() 裡，要改次數兩邊都要改
   ```

> anon key 是設計上可公開的金鑰，放在前端沒問題——真正的把關在資料庫的 RLS 與函式。`supabase/email-templates/magic-link.html` 已不再使用（舊的魔法連結信件範本），可留著備用或直接刪除。

> 廣告視窗是純前端做的示意版（兩張靜態圖輪流顯示，2 秒後才能按 X 關閉並領取加轉，點圖會另開分頁到廣告連結），沒有接任何真的廣告 SDK 或聯盟連結。要正式上線的話，改 `script.js` 裡的 `AD_CREATIVES` 陣列（每個項目是 `src`／`href`／`copy`），或改接 Google AdSense/AdMob 之類的廣告 SDK。
>
...
部署後開啟網頁 → 輸入 email 與密碼 → 點「登入 / 註冊」（沒帳號會自動建立）→ 開始轉盤。轉盤正上方會顯示「今天還可轉 N 次」。轉盤上方可以切換「預設轉盤」／「共享轉盤」；右上角可以建立或加入飯搭子圈、進我的檔案。
