...
- **看廣告多轉一次**：3 次用完後可看一次 3 秒的模擬廣告（遠銀 Bankee），換得當天 +1 次額度，每人每天限領一次，額度同樣由後端強制執行
...
> 廣告視窗目前是純前端做的示意版（3 秒倒數 + 一張寫死的 Bankee 文字/漸層卡片），沒有接任何真的廣告 SDK 或聯盟連結。要正式上線的話，把 `index.html` 裡 `.ad-banner` 那塊換成你實際的廣告素材／聯盟連結，或改接 Google AdSense/AdMob 之類的廣告 SDK。
