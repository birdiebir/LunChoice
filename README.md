...
4. 後台 **Authentication → Providers → Email** → 關閉 **Confirm email**（不勾）。這樣註冊（`signUp`）會直接建立並登入帳號，完全不需要寄送任何驗證信、不用碰 SMTP／寄件帳號設定。
   - 如果保留 **Confirm email** 開啟，Supabase 仍會用內建（或你自訂）的寄件服務寄驗證信，註冊後要先到信箱點連結才能登入。
...
部署後開啟網頁 → 輸入 email 與密碼 → 點「登入」（沒帳號先點「註冊」）→ 開始轉盤。右上角會顯示「今天還可轉 N/3 次」。
