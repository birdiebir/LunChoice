...
- **看廣告多轉一次**：3 次用完後轉盤上會出現提示，點下去播一張 3 秒靜態廣告圖（遠銀 Bankee，示意用），看完換得當天 +1 次額度，每人每天限領一次，額度同樣由後端強制執行
...
2. 後台 **SQL Editor** → 貼上 `supabase/schema.sql` 整份執行（建立 `spins`／`ad_bonuses` 表與 `record_spin`／`spin_status`／`claim_ad_bonus` 函式；整份可重複執行，改版後重貼一次即可更新）。
...
> 廣告視窗目前是純前端做的示意版（3 秒倒數 + `assets/ad-placeholder.png` 這張隨便生成的漸層佔位圖），沒有接任何真的廣告 SDK 或聯盟連結。要正式上線的話，把 `assets/ad-placeholder.png` 換成實際的廣告圖片（同檔名直接覆蓋即可，或改 `index.html` 裡 `.ad-media img` 的 `src`），或改接 Google AdSense/AdMob 之類的廣告 SDK。
